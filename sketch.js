...
var END=0;
var gameState=1;
var k,fruit1,fruit2,fruit3,fruit4,monster,monster2;
...
function setup() { 
  ...
  k = createSprite(100,200,20,20);  
  k.scale=0.72;
  k.addImage(swordImage);
...
function draw() {
 ...
 k.y=World.mouseY;
  k.x=World.mouseX;   
   if(fruitGroup.isTouching(k)) {
    fruitGroup.destroyEach();
    score=score+2;
   }
     else if(enemyGroup.isTouching(k)) {  
      gameState = END;
      fruitGroup.destroyEach();
      enemyGroup.destroyEach();
      fruitGroup.velocityX=0;
      enemyGroup.velocityX=0;
      k.addImage(gameoverImage);
      k.scale=1.5;
      k.x=300;
      k.y=300;
    }
...
function E() { 
   if(World.frameCount%200 === 0) { 
     monster=createSprite(60,200,20,20);
